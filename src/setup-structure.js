...
async function setupProjectStructure() {
  ...
  try {
    ...
    await fs.writeFile(path.join(projectRoot, 'public/assets/images/.gitkeep'), '', 'utf8');
    await fs.writeFile(path.join(projectRoot, 'public/assets/fonts/.gitkeep'), '', 'utf8');

    console.log(
      `\n${colors.brightGreen}✨ Estrutura do projeto criada com sucesso!${colors.reset}`
    );
    console.log(`\n${colors.brightMagenta}📊 Resumo:${colors.reset}`);
    console.log(`  ${colors.yellow}- ${folders.length} pastas criadas${colors.reset}`);
    console.log(`  ${colors.yellow}- ${Object.keys(files).length} arquivos criados${colors.reset}`);
  } catch (error) {
    console.error(`${colors.brightRed}❌ Erro ao criar estrutura:${colors.reset}`, error);
    throw error;
  }
}
...
async function cleanupProjectStructure() {
  ...
  try {
    ...
    console.error(`${colors.brightRed}❌ Erro ao limpar estrutura:${colors.reset}`, error);
    throw error;
  }
}
...
async function showMenu() {
  ...
  console.log(`  ${colors.brightRed}0${colors.reset} - ${colors.white}Sair${colors.reset}\n`);

  const choice = await question(`${colors.cyan}Digite sua escolha: ${colors.reset}`);
  ...
  try {
    ...
    console.error(`\n${colors.brightRed}❌ Erro:${colors.reset}`, error.message);
